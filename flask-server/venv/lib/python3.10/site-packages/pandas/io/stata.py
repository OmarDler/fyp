...
def _cast_to_stata_types(data: DataFrame) -> DataFrame:
    ...
    for col in data:
        ...
        is_nullable_int = isinstance(
            data[col].dtype, (_IntegerDtype, BooleanDtype))
        orig = data[col]
...
class StataReader(StataParser, abc.Iterator):
    ...
    def _read_header(self) -> None:
        ...
        self.has_string_data = len(
            [x for x in self.typlist if type(x) is int]) > 0
    ...
    def _read_new_header(self) -> None:
        ...
        self._seek_vartypes = (
            struct.unpack(self.byteorder + "q",
                          self.path_or_buf.read(8))[0] + 16
        )
        self._seek_varnames = (
            struct.unpack(self.byteorder + "q",
                          self.path_or_buf.read(8))[0] + 10
        )
        self._seek_sortlist = (
            struct.unpack(self.byteorder + "q",
                          self.path_or_buf.read(8))[0] + 10
        ...
        self._seek_formats = (
            struct.unpack(self.byteorder + "q",
                          self.path_or_buf.read(8))[0] + 9
        )
        self._seek_value_label_names = (
            struct.unpack(self.byteorder + "q",
                          self.path_or_buf.read(8))[0] + 19
        )
        ...
        self.data_location = (
            struct.unpack(self.byteorder + "q",
                          self.path_or_buf.read(8))[0] + 6
        )
        self.seek_strls = (
            struct.unpack(self.byteorder + "q",
                          self.path_or_buf.read(8))[0] + 7
        )
        self.seek_value_labels = (
            struct.unpack(self.byteorder + "q",
                          self.path_or_buf.read(8))[0] + 14
        )
    ...
    def _read_old_header(self, first_char: bytes) -> None:
        ...
        self._set_encoding()
        self.byteorder = (
            struct.unpack("b", self.path_or_buf.read(1))[
                0] == 0x1 and ">" or "<"
        )
        ...
        self.nvar = struct.unpack(
            self.byteorder + "H", self.path_or_buf.read(2))[0]
        self.nobs = self._get_nobs()
