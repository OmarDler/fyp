...
def create_and_load_iris(conn, iris_file: Path, dialect: str):
    ...
    with iris_file.open(newline=None) as csvfile:
        reader = csv.reader(csvfile)
        header = next(reader)
        params = [{key: value for key, value in zip(
            header, row)} for row in reader]
...
class TestSQLApi(SQLAlchemyMixIn, _TestSQLApi):
    ...
    def test_query_by_select_obj(self):
        ...
        name_select = select(iris_select).where(
            iris.c.Name == bindparam("name"))
        ...
        if _gt14():
            with Session(self.conn) as session:
                with session.begin():
                    ...
                    session.flush()
                    df = sql.read_sql_query(
                        sql=select(Temporary.spam), con=conn)
        else:
...
class TestXSQLite:
    ...
    def test_keyword_as_column_names(self):
        ...
        assert sql.to_sql(df, con=self.conn,
    ...
    def test_onecolumn_of_integer(self):
        ...
        assert sql.to_sql(mono_df, con=self.conn,
